...
def node_synthesizer(state: AgentState) -> AgentState:
    ...
    【格式化高危红线（必读）】：
    为了防止被机器质检误杀，你在使用 <raw> 标签时必须遵守以下格式铁律：
    
    1. 绝对禁止包含单位！单位必须写在标签外面。
       ❌ 错误：<raw>199单</raw>  |  ✅ 正确：<raw>199</raw> 单
    
    2. 绝对禁止添加千分位逗号！
       ❌ 错误：<raw>8,418,543.22</raw>  |  ✅ 正确：<raw>8418543.22</raw>
       
    3. 绝对禁止私自补齐或删减小数点！底层是几位就是几位。
       ❌ 错误：将底层 9421810.6 写成 <raw>9421810.60</raw>
    ...
    【🚨 极其容易犯错的致命陷阱 —— 你必须死死记住！】
    历史记录显示，你极容易在“汇总总盘子”时犯下死罪！
    
    场景重现：底层查出了双十一当天的GMV是 9576400.8，预热期的GMV是 11759214.86。
    你想在报告里写总GMV，于是你聪明地把它们加了起来，得到了 21335615.66。
    
    ❌ 致命错误做法：你觉得这是客观事实，于是写成了 <raw>21335615.66</raw>。
    => 结果：立刻被质检机拦截！因为底层根本没有直接给出 21335615.66 这个字符串！
    
    ✅ 唯一正确做法：只要这个数字是你用加法拼凑出来的，无论它多么符合业务逻辑，你都必须老老实实写成 <calc>21335615.66</calc>！
    
    记住：<raw> 的尊严在于“原封不动、纯纯的复制粘贴”。只要经过了加减乘除，统统是 <calc>！
